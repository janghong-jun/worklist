...
var Worklist = (function(){
	...
	var setMap = function(options){
		_manmonth = {
			projectStartDate : "2021/6/1"
			, projectEndDate : "2022/6/30"

			//-----------------------------------
			// [프로젝트가 1년 넘어갈때] :연단위 Change 위한 FlagNum(추후수정예정) 
			// yearChangeNum : 5  --> 연단위 Change : 6~12월 : 2021, 1~5월 : 2022
			// >>  yearChangeNum 검색하여 해당 연도 수정 필
			, yearChangeNum : 5
			//-----------------------------------
			, holiday: [
				//국경일 & 공휴일
				{date: '2022/1/1', label: '양력설'}
				, {date: '2022/3/1', label: '3.1절'}
				, {date: '2022/5/1', label: '근로자의날'}
				, {date: '2022/5/5', label: '어린이날'}
				, {date: '2022/6/6', label: '현충일'}
				, {date: '2022/7/17', label: '제헌절'}
				, {date: '2022/8/15', label: '광복절'}
				, {date: '2022/10/3', label: '개천절'}
				, {date: '2022/10/9', label: '한글날'}
				, {date: '2022/12/25', label: '크리스마스'}

				// - 
				, {date: '2022/1/31', label: '설연휴'}
				, {date: '2022/2/1', label: '설연휴'}
				, {date: '2022/2/2', label: '설연휴'}
				, {date: '2022/5/8', label: '부처님오신날'}// -4/8
				, {date: '2022/9/9', label: '추석연휴'}
				, {date: '2022/9/10', label: '추석연휴'}
				, {date: '2022/9/11', label: '추석연휴'}
				, {date: '2022/9/12', label: '추석연휴'}

				// Etc
				, {date: '2022/3/9', label: '20대 대통령 선거일'}
				, {date: '2022/6/1', label: '2022 지방선거'}
			]
			, corder:[
				{
					id:'corder01'
					...
					, endDate: '2022/9/30'
					, vacation : [
						{date: '2022/7/15', label: '조퇴', day:0.5}
					]
				}
				, {
					id:'corder02'
					...
					, endDate: '2022/7/29'
					, vacation : [
						{date: '2022/4/29', label: '연차', day:1}
						, {date: '2022/5/26', label: '연차', day:1}
						, {date: '2022/6/16', label: '연차', day:1}
					]
				}
				, {
					id:'corder03'
				...
				, {
					...
					, name:'장홍준'
					, startDate: '2022/4/25'
					, endDate: '2022/8/31'//7/19: 8/16 > 8/31 변경
					, vacation : [
						{date: '2022/5/10', label: '연차', day:1}//날짜 다시확인 & 반차
						, {date: '2022/5/16', label: '반차', day:0.5}
						, {date: '2022/7/11', label: '반차', day:0.5}
					]
				}
				, {
					id:'corder05'
					, name:'문소연'
					, startDate: '2022/4/25'
					, endDate: '2022/7/29'//7/19: 8/16 > 7/29 변경
					, vacation : [
						{date: '2022/5/25', label: '병가', day:1}
						, {date: '2022/6/15', label: '연차', day:1}
						, {date: '2022/7/19', label: '연차', day:1}
					]
				}
			]
		}
		...
		, _g = {
			...
			, localStorage :  Storage.get(_g.projectID) || false
			...
			, dataObj : {
				...
				, folding_depth : null //2~n 까지 폴딩 버튼 삽입(Default :2미.만 안보임)
				, debug_mode : false
				, test_mode : false
			...
			, dateObj :{
				seperate : '/' //날짜 포맷 형식
				, seperates_changes : ['-', '_', '.', ',', '/'] //seperate로 변경할 날짜 포맷 형식 형식들
				, zeroFill : 2 // 1: 9/9, 2: 09/09
				// , yearChangeNum : 5// 연단위 Change : 6~12월 : 2021, 1~5월 : 2022
				, yearChangeNum : _manmonth.yearChangeNum// 연단위 Change : 6~12월 : 2021, 1~5월 : 2022
				, yearPrefix : '20'
				, datePeriodFlag : false // false : 전체기간, true: 기간설정
			}
			, searchOption : {
				viewMode :"select" //Search_head 관련 select or  button //@TODO : search Option 이 Filter 일경우(셀렉트박스인지 버튼타입인지 구분)
				, all_target_mode : true
			}
			, logObj : {
				isShowAll : false // Log 한줄 보기 또는 전체 보기 Flag
			}
			, layerPopUrl :{
				guide : '../_pub_guide/guide_sample/layerPop_worklist.html' //레이어팝업 가이드 URL
				, server : '../../' + _g.fileDir + '/' // 서버에서 레이어팝업 호출시 경로
				, local : '../../webApp/'  +_g.fileDir + '/' //로컬에서 레이어팝업 호출시 경로
			}
		...
		, navMap = {
			...
			, overNum2 : -1
			, overNum3 : -1
			, activeNum : _g.navNum //초기 전체 보기 활성화(View All)
			, activeNum2 : false
			, activeNum3 : false
		}
		, dataMap = {
	...
	var Storage = (function(){
		var set = function(obj, val){
			_set(_g.projectID, JSON.stringify(_g.storageObj));
		}

		, get = function(obj){
			return _get(_g.projectID);
		}
		...
		, initModule = function(){
			_g.localStorage = get(_g.projectID) || false; 
			_g.storageObj= _g.localStorage ? JSON.parse(_g.localStorage) : _g.defaultObj;

			_g.defaultObj[_g.device] = _g.defaultObj[_g.device] || {};
			_g.defaultObj[_g.device].setting = _g.defaultObj[_g.device].setting || {};
			_g.defaultObj[_g.device].colorBox = _g.defaultObj[_g.device].colorBox || {};
			_g.defaultObj[_g.device].headerFolding = _g.defaultObj[_g.device].headerFolding || {};

			_g.storageObj[_g.device] = _g.storageObj[_g.device] || {};
			_g.storageObj[_g.device].setting = _g.storageObj[_g.device].setting || {};
			_g.storageObj[_g.device].colorBox = _g.storageObj[_g.device].colorBox || {};
			_g.storageObj[_g.device].headerFolding = _g.storageObj[_g.device].headerFolding || {};

			//비교
			// _g.compareDefaultObj = JSON.stringify(_g.defaultObj)
			// console.log(JSON.stringify(_g.defaultObj))
			// _g.storageObj.saveDefault = _g.compareDefaultObj


			if(_g.is_storage){//기존 스토리지 제거
				//remove('worklistnew');
			}
		}
	...
	var Setting = (function(){
		...
		, init = function(_arg){
			...
			if(!_g.storageObj[_g.device].setting[target]) _g.storageObj[_g.device].setting[target] =[];
			if(!_g.defaultObj[_g.device].setting[target]) _g.defaultObj[_g.device].setting[target] =[];

			//동적 Object 생성
			_g.settingObj[setOption] ={};

			// Chk 저장
			$chkObj.each(function(i){
				var name = $(this).attr('name')
				_g.settingObj[setOption][name] = false;
			});

			//init
			var objCnt =0;
			for(var name in _g.settingObj[target]){
				if(!!!name) return;
				...
				// 스토리지 객체 데이터 저장
				_g.settingObj[target][name] = _g.storageObj[_g.device].setting[target][objCnt];

				if(Setting.bool(_g.storageObj[_g.device].setting[target][objCnt])) $chkbox.prop('checked', true);
				else $chkbox.prop('checked', false);

				if(_g.settingObj[target][name] === "O") {
				...
				//강제 설정
				if(_g.defaultObj[_g.device].setting[target][objCnt] === "X") {
					_g.settingObj[target][name] = "X";
					$chkbox.attr('disabled', true);
					$chkbox.prop('checked', false);
				}

				if(_g.defaultObj[_g.device].setting[target][objCnt] === "O") {
					_g.settingObj[target][name] = "O";
					$chkbox.attr('disabled', true);
					$chkbox.prop('checked', true);
				}
				...
				if(target =='option' && $chkbox.hasClass('data_group')){
					if(_g.storageObj[_g.device].setting[target][0] == true) { //Data 가 체크 되 어 있을시 하위 그룹 선택가능
						$chkbox.attr('disabled', false);
					}

					if(_g.defaultObj[_g.device].setting[target][objCnt] === "X") { // X 일 경우에도 체크 할수 없음
						$chkbox.attr('disabled', true);
						$chkbox.prop('checked', false)
		...
		, optionCheck = function(_arg){
			...
			$chkObj.each(function(i){
				$(this).change(function(){
					...
					//강제 적용은 스토리지 저장 제외
					if(_g.defaultObj[_g.device].setting[target][index] === "X") return;
					if(_g.defaultObj[_g.device].setting[target][index] === "O") return;

					if($(this).prop('checked')) str = true;
					else if($(this).attr('disabled', false)) str = false;

					_g.storageObj[_g.device].setting[target][index] = str;
					// console.log('change2: ', _g.storageObj[_g.device].setting[target])
				});
			});

			//option > Data 연계되어 있는 놈들
			if(target =='option'){
				$('input[name="data"]').change(function(){
					...
					_g.storageObj[_g.device].setting[target][0]= str;
				});
			}
		}
		...
		, _appendTableShow_Search = function(){
			...
			rMap.tab_content.eq(0).find('> table > thead > tr th').each(function(i){
				...
				_g.tableHeadObj.label[i] = labelName;
				_g.tableHeadObj.class[i] = className;
			...
			setTimeout(function(){
				_g.settingObj.table2 = {}
				for(var i= 0; i < _g.tableHeadObj.class.length; i++){
					var className = _g.tableHeadObj.class[i];

					// Setting 테이블만 보이게 : 나머지는 삭제
					// if(!!!Setting.bool(_g.settingObj.table["t_"+className])) rMap.tab_content.find('.'+className).hide();
					if(!!!Setting.bool(_g.settingObj.table["t_"+className])) rMap.tab_content.find('.'+className).remove();
				}
			},10)
		}
	...
	var Search = (function(){
		...
		var setMap = function(_arg){
			thisMap ={
				// all_target_mode : true// true : 전체 테이블 카테고리 , false: 해당 테이블 카테고리
				all_target_mode : _g.searchOption.all_target_mode// true : 전체 테이블 카테고리 , false: 해당 테이블 카테고리
				, searchOn : false
			}
		}
		...
		, set = function(_arg, _callback){
			...
			_g.settingObj.search_classes =[];
			if(targetClass == 'tag') $target_search = $target_search.find('> span'); //[tag]
			if(targetClass == 'log') $target_search = $target_search.find('> dl'); //[log]

			$target_search.each(function(index, el) {
				if(targetClass == 'tag') ARfind.push($(this).text()); //[tag]
				else if(targetClass == 'log') ARfind.push($(this).find('> dt').text()); //[log]
				else ARfind.push($(this).text());
				// else {
				// 	ARfind.push($(this).not('[data-ui="more"]').text());
				// }
				// if(targetClass == 'log') $desc = $trs.find('> *:not(.btn_more)');//[log]
		...
		, eventHandler = function($target, targetClass, _arg){
			$('select', $target).off().on('change', function(e){
				...
				var $desc;
				if(targetClass == 'tag') $desc = $trs.find('> span');//[tag]
				if(targetClass == 'log') $desc = $trs.find('> dl');//[log]
				...
				if(val == 'all'){
					...
					if(targetClass == 'tag') Component.active('All');//[tag]

					return;
				}else{
					var totalCnt=0;

					if(targetClass == 'tag') Component.active(val_text);//[tag]
					...
					$trs.each(function(i){
						...
						//[log]
							...
							$tr.find('.'+targetClass).find('> dl').each(function(){
								if(!_g.logObj.isShowAll) $(this).hide();
								if($(this).find('> dt').text() === val_text){
									if(!_g.logObj.isShowAll) $(this).show();//desc
									$tr.show().addClass('sortOn');
									totalCnt++;
								}
							});
		...
		, init = function(){
			rMap.tab_content.eq(0).find('> table > thead > tr th').each(function(i){
				...
				if(Setting.bool(_g.settingObj.search[className])){
		...
		, reset = function(){
			if(_g.filterObj.delete) {// 삭제 포함 : 전체
	...
	var Search_header = (function(){
		...
		, set = function(_arg, _callback){
			var labelName =_arg.label
				, targetClass = _arg.class
				, sortType = _arg.sort
				, viewMode = _arg.viewMode ? _arg.viewMode : _g.searchOption.viewMode // Type : select  or button
		...
		, init =function(){
			...
			if(Setting.bool(_g.settingObj.search.corder)) set({label: '담당자', class: 'corder', sort: 'string'}, function(){});
			if(Setting.bool(_g.settingObj.search.ddate)) set({label: '예정', class: 'ddate', sort: 'date'}, function(){});
			if(Setting.bool(_g.settingObj.search.rdate)) set({label: '완료', class: 'rdate', sort: 'date'}, function(){});
			if(Setting.bool(_g.settingObj.search.pdate)) set({label: '검수대상', class: 'pdate', sort: 'string'}, function(){});
			if(Setting.bool(_g.settingObj.search.mdate)) set({label: '고객검수', class: 'mdate', sort: 'string'}, function(){});
		}

		, initModule =function(){
			init();///header .info_section에 삽입 : 추후 작업예정
		}
	...
	var Data = (function(){
		...
		var var_init = function(){
			...
			_g.dataObj.depSize = depSize;
			_g.dataObj.folding_depth = depSize;

			//Object Setting : Onum
			for(var i=1; i<= _g.dataObj.depSize; i++){
		...
		, dateFormat = function(_str){
			//공백제거 및 Seperate Chage
			var dateStr = _str.replaceAll(' ', '');
			for(var i=0; i<=_g.dateObj.seperates_changes.length -1; i++){
				if(_g.dateObj.seperates_changes[i] != _g.dateObj.seperate){
					dateStr = dateStr.replaceAll(_g.dateObj.seperates_changes[i], _g.dateObj.seperate);
				}
			}
			var ARstr = dateStr.split(_g.dateObj.seperate);
			var strTmp = '';

			for(var i = 0; i<ARstr.length; i++){
				if(!isNum(ARstr[i])) return;//숫자 아닐때
				if(i == ARstr.length-1) strTmp += Number(ARstr[i]).zf(_g.dateObj.zeroFill); //일 뒤엔 '/' 삽입 안함
				else strTmp += Number(ARstr[i]).zf(_g.dateObj.zeroFill) + _g.dateObj.seperate;
			}
		...
		, dateChange = function($date){
			var dateStr = $date.text()
				...
				, strFullTmp = ''
				, monthNum = Number(formatStr.split(_g.dateObj.seperate)[0]) 
			;

			//추후 yearChangeNum 변경 예정
			if(monthNum < _g.dateObj.yearChangeNum) strFullTmp = '2022'+_g.dateObj.seperate+formatStr;
			else strFullTmp = '2022'+_g.dateObj.seperate+formatStr;

			if(isNull(formatStr) || isNull(dateStr) || dateStr == _g.dateObj.seperate || dateStr.length < 2) {
		...
		//Table Data 초기화
		, data_init_ORI = function(){
			var tmp = {}
				, depNum = _g.dataObj.depSize
			;
			...
			rMap.tab_content.each(function(i){
				...
				$(this).find('tbody >tr').each(function(i, row){
					for(var jj = 2; jj <= depNum; jj++){
						var $depths = $(this).find('> td.depth'+jj); //depth
						if($depths.find('a').length > 0 || $('body').data('page') !='worklist'){ // 대표 가이드 링크 예외처리

						}else{
							$depths.text(lrtrim($depths.text()));
						}
					}

					var $page = $(this).find('> td.page') //Page
						, $corder = $(this).find('> td.corder') //담당자
						, $pdate = $(this).find('> td.pdate') //검수대상
						, $mdate = $(this).find('> td.mdate') //고객검수
						, $ddate = $(this).find('> td.ddate') //예정일(날짜)
						, $rdate = $(this).find('> td.rdate') //완료일(날짜)
						, $log = $(this).find('> td.log') //Log
					;

					$page.text(lrtrim($page.text()));
					$corder.text(lrtrim($corder.text()));
					$pdate.text(lrtrim($pdate.text()));
					$mdate.text(lrtrim($mdate.text()));
					dateChange($ddate);
					dateChange($rdate);
		...
		, data_init = function(){
			...
			// rMap.tab_content.find('tbody >tr').each(function(i, row){
			...
			rMap.tab_content.each(function(i){
				...
				//좌우 공백제거 & //날짜 포맷 
				// $(this).find('tbody >tr').each(function(i, row){
				// 	var $td = $(this).find('>td:not(.log)');
				// 	var $log = $(this).find('>td.log');
				// 	$td.each(function(){
				// 		$(this).html(lrtrim($(this).html()));
				// 		var className = $(this).attr('class');
				// 		if(className =='num' || className =='ddate' || className =='rdate'){
				...
				$(this).find('tbody >tr').each(function(ii, row){
						...
						for(var o=2; o<=depNum; o++){
							if(ARcnt[ii][o-1] == 0){//2Depths : 첫번째 포지션 위치 저장
								...
								if(_g.dataObj.test_mode) $(this).attr('data-first'+o, true); //추후 삭제
							}
							tmp['num'+o]++;
	 ...
	 * View
	 * --------------------------------------
	 * viewAll 인지 viewOne 인지에 따라 Change : 작업예정
	 */
	var View = (function(){
		var viewAll = function(_target){
			_g.page.is_viewAll = true;

			// _g.searchOption.all_target_mode = true;
			// Search.initModule();
		}
		, viewOne = function(_target){
			_g.page.is_viewAll = false;

			// _g.searchOption.all_target_mode = false;
			// Search.initModule();
		}
		, watch = function(){
			if(_g.page.is_viewAll){
				viewAll();
			}else{
				viewOne();
			}
	...
	var Nav = (function(){
		var is_fixDep2Pad = true //2Depths 고정여부 //contentr와 nav 상태분리
			...
			, is_init_depth1 = true // 초기 1Depth 활성화 적용했는지 여부
		...
		var _show = function(_target){
			...
			//@2Depths 메뉴가 보여질 경우 아래 실행
			if(Setting.bool(_g.settingObj.data.nav_depth2) != true) return;
			...
			rMap.tab_navList.each(function(i){
				...
				$depth1.find('> a').on({
					...
					, click : function(e){
						...
						//Hash Mode
						if(!!!_g.page.scrollMode){
							//전체보기
							if(navMap.activeNum == _g.navNum){
				...
				$('.subNav li a', $depth1).on({
					...
					,'click' : function(e){
						...
						//View All 아닐경우 2Depth 클릭시 해당 1Depth로 이동후 scrollTop
						if((navMap.activeNum != _g.navNum)  && (navMap.overNum != navMap.activeNum)){
							// _g.page.is_viewAll = false;
							View.viewOne(e.target);
		...
		, goContent = function(){
			...
			if(_g.page.is_viewAll){
				...
				if(navMap.overNum < 0 ) navMap.overNum = _g.navNum - 1;
				if(navMap.overNum > _g.navNum - 1) navMap.overNum = 0;
		...
		//View all(전체보기) 모드일 경우: 초기 화면 포지션에 위치에 따른 네비게이션 active
		, initDepth1 = function(){
			headerGapPad = $('.header').height() + 8;
			...
			rMap.tab_content.each(function(i){
				...
				//현재 top 포지션 기억해서 배열과 비교 현재 인덱스 저장
			...
			// console.log('catePos :', ARcontentTop, ARcontentTop.length, curPos, navMap.overNum);
		}

		, initDepth2 = function(){
			//@TODO : 화면 작을때 Depth1 메뉴 스크롤 생기게
			//서브 네비 셋팅
			rMap.tab_navList.each(function(i){
				//2Depth  강제로 안보이게 할경우
		...
		*/
		, setWidth = function(){
			//모바일 1Depths 메뉴 가로 스크롤 화면일꼉우 small Size Nav로 변경
			var breakpoint = 960;
			function navWidth(){
				var mW = 0;
				var gap = 2;
		...
		, initModule = function(){
			...
			if(!_g.is_mobile && Setting.bool(_g.settingObj.data.nav_depth2)){
				initDepth2();
			}
			...
			if(Setting.bool(_g.settingObj.option.scroll)) _g.page.scrollMode = true;
			else _g.page.scrollMode = false;

			setTimeout(function(){
				initDepth1();//top position setting
			},500);
			
			if(_g.page.scrollMode){//scrollMode
				location.hash = '';
				navMap.activeNum = navMap.overNum = _g.navNum;
				rMap.tab_navList.eq(_g.navNum).find('>a').text('↑top');
			}else{//HashMode
				...
				if(!!$hash){
					...
					navMap.overNum = navMap.activeNum = _g.navNum;
				}
				_show();
				rMap.tab_navList.eq(_g.navNum).find('>a').text('View All');
			}
	...
	var TableFolding = (function(){
		...
		var init = function(){
			var depNum = _g.dataObj.depSize -1;// 마지막 폴딩은 삭제
	...
	var HeaderFolding = (function(){
		...
		var appendLayout = function(){
			// rMap.info_section.find('>div h2').append(' <a href="javascript:void(0);" class="btn_close_info"> - </a>');
		...
		, eventHandler = function(){
			$toggleBtn.on('click', function(){
				if(rMap.body.hasClass('headerFloating') ) {// top > 0
					...
					_g.storageObj[_g.device].headerFolding.headerClose = headerT_Flag;
					Storage.set();
					setHeader();
				}
				return false;
		...
		, localStorage = function(){
			if(_g.storageObj[_g.device].headerFolding.headerClose){
				headerT_Flag = true;
				rMap.info_section.stop().hide();
			}else{
				headerT_Flag = false;
	...
	var ColorBox = (function(){
		...
		var setMap=function(){
			...
			cbVar = {
				...
				, outerGap : 22 //45
				, curSize :  _g.storageObj[_g.device].colorBox.curSize
				, resizeMode : _g.storageObj[_g.device].colorBox.resizeMode ? _g.storageObj[_g.device].colorBox.resizeMode : 'Resize'
				, ARresize :[]
		...
		, colorBoxHandler = function(){
			...
			function scrollbarHandler(){
				...
				if(cbVar.curSize <= breakPoint){
					// $iframe.contents().find('html').addClass('mobile');
			...
			function resizeBtnTrigger(){
				...
				_g.storageObj[_g.device].colorBox.resizeMode ='Resize';
				Storage.set();
			...
			function fullsizeBtnTrigger(){
				...
				_g.storageObj[_g.device].colorBox.resizeMode ='Fullsize';
				Storage.set();
			...
			fMap.control_btn.each(function(){
				...
				$btn.on({
					...
					, click : function(){
						...
						cbVar.curSize = dataW;
						_g.storageObj[_g.device].colorBox.resizeMode ='Resize';
						_g.storageObj[_g.device].colorBox.curSize =cbVar.curSize;
						Storage.set();
			...
			rMap.tab_content.find('tr').each(function(i){
					...
					|| $(this).css("display") == 'none')
				{
					...
					if($(this).hasClass('layer')){
						if(_g.url_info.search('file') != -1) goURL = _g.layerPopUrl.local + goURL; //로컬로 볼경우
						else goURL = _g.layerPopUrl.guide + '?' + _g.layerPopUrl.server + goURL; //서버에서 볼경우
					}
	...
	var AppendNum = (function(){
		...
		var appendNumber = function(){
			...
			$('body').attr('data-childOption', _g.filterObj.group)// child Option일 경우 td.Num 폰트 스타일

			$contentTR.each(function(){
				//포함되지 않는 케이스
				if($(this).hasClass('nocnt')) {
					_g.totalNoCnt = tmpNoIndex+1;
					if(_g.is_firstNum) $('td:first-child', $(this)).before('<td class="num"><span class="number">- '+ _g.totalNoCnt + '</span></td>');//init
					else $('td:first-child', $(this)).text(_g.totalNoCnt);
					tmpNoIndex = _g.totalNoCnt;
					return;
				}

				if(_g.filterObj.group && $(this).hasClass('child')){
					_g.totalCnt = tmpIndex;
					subIndex = subIndex +1;
					if(_g.is_firstNum) $('td:first-child', $(this)).before('<td class="num"><span class="number">'+ _g.totalCnt +"-"+subIndex+'</span></td>');//init
				}
				// 2차 오픈 추가 : 2020-03-10
				/*else if(! _g.filterObj.open2 && $(this).hasClass('open2')){
					return
				}*/
				else{
					_g.totalCnt = tmpIndex+1;
					subIndex = 0;
					if(_g.is_firstNum) $("td:first-child", $(this)).before('<td class="num"><span class="number">'+ _g.totalCnt +'</span></td>');//init
					else $('td:first-child', $(this)).text(_g.totalCnt);
					tmpIndex = _g.totalCnt;
				}
			});

			_g.is_firstNum = false;
		}
	...
	var Filter = (function(){
		...
		function open2Handler (){
			// console.log('open2')
			if(_g.filterObj.open2) $('.open2').removeClass('open2-off')
		...
		function calculator(){
			...
			$contentTR.each(function(index){
				...
				if($('body').hasClass('linkShow')){//2021.1.11 : 수정
					...
					if(!rdateTxtFlag || $(this).find('td.rdate').text() =='-') {
			...
			$totalBtn.find('span').text('전체 : ' + _g.totalCnt);

			if(_g.filterObj.open2){
				$open2Btn.show();
			...
			var realPercent = (resultCnt/_g.totalCnt)*100;
		...
		function calculator_cate(){
			...
			$('.info_calc').each(function(){
				...
				var open2Cnt = 0 //2차 오픈 관련: 2020-03-10
					...
					, realPercent
				;
				_g.totalCnt = Number($table.find('tr').size()) //전체수
					...
					if($(this).hasClass('del')){
						if(_g.filterObj.delete) {
							if($(this).hasClass('child')){
								_g.totalCnt--;//삭제제외(Default)
							}
						}//삭제포함
						else _g.totalCnt--;//삭제제외(Default)
					}

					//Group
					if($(this).hasClass('child')){
						if(_g.filterObj.group) {
							if($(this).hasClass('del')) {}
							else _g.totalCnt--;//Yes(Default)
						}
					}

					//open2
					if($(this).hasClass('open2')){
						if(_g.filterObj.open2) {}//1차
						else _g.totalCnt--;//전체(Default)
					}

					//완료일 Count
					var resultDateCell = $(this).find('td.rdate');
					var tdStr = String(resultDateCell.text());
					if(!tdStr){}
					// 작업완료일 기입시
					else {
						resultCnt++;
						...
						//Delete
						if($(this).hasClass('del')){
							if(_g.filterObj.delete) {}//삭제포함
							else resultCnt--;//삭제제외(Default)
						}

						//Group
						if($(this).hasClass('child')){
							if(_g.filterObj.group) resultCnt--; //Yes(Default)
						}
				...
				var percent = (resultCnt/_g.totalCnt)*100;
				if(String(percent).split('.')[1]) percent = percent.toFixed(2);

				if(isNaN(percent)) percent = 0;
				$(this).html(resultCnt + ' / ' + '<strong style="color:#888">'+_g.totalCnt +'</strong> = ' + percent + '%');
			});
		}
		...
		function eventReset($onBtn){
			...
			$contentTR.each(function(index){
				...
				//6.삭제
				if(btnName =="del" && _g.filterObj.delete){
					trHide($(this));//show
				...
				//00. 2차 오픈
				if(btnName =="open2" && _g.filterObj.open2){
					trHide($(this));//show
				...
				if(btnName =="result" || btnName =="result_ex"){
					if(rdateTxtFlag){//완료
						if(btnName =="result"){
							...
							if(! _g.filterObj.delete && $(this).hasClass('del')) trHide($(this));//hide //Delete 제외
						}
					} else{//미완료
						if(btnName =="result_ex"){
							trShow($(this));//Delete 포함
							if(! _g.filterObj.delete && $(this).hasClass('del')) trHide($(this));//Delete 제외
						}
					}
					return
				}
		...
		function eventHandler(){
			...
			$('input[name="ch_del"]').change(function(){
				_g.filterObj.delete = $(this).val() == "true" ? true : false;
				update();
			});

			//그룹
			$('input[name="ch_group"]').change(function(){
				_g.filterObj.group =  $(this).val() == "true" ? true : false;
				update();
			...
			$('input[name="ch_child"]').change(function(){
				_g.filterObj.child = $(this).val() == "true" ? true : false;
				if(_g.filterObj.child) $childItem.show();
				else $childItem.hide();
			});

			// /2차 오픈 관련 Check
			$('input[name="ch_open2"]').change(function(){
				_g.filterObj.open2 = $(this).val() == "true" ? true : false;
				open2Handler();
				resetModule();
			});
		...
		var option_init = function(){
			_g.filterObj.delete = $('input[name="ch_del"]:checked').val() == "true" ? true : false;
			_g.filterObj.group = $('input[name="ch_group"]:checked').val() == "true" ? true : false;
			_g.filterObj.open2 = $('input[name="ch_open2"]:checked').val() == "true" ? true : false;// 2차 오픈 추가 : 2020-03-10
			_g.filterObj.child = $('input[name="ch_child"]:checked').val() == "true" ? true : false;// Child 추가 : 2021-11
			// update();
		}
		...
		, initModule = function(){
			...
			$(document).off('click.popclose').on('click.popclose', '.is_popOn .dimmed, .is_popOn .btn_close', function(){
				Popup.close(function(){
					if(_g.focusObj) _g.focusObj.addClass('focus');
				});
			});
		}
	...
	var Layout = (function(){
		...
		, setLocation = function(){
			rMap.tobe_section.find('a').each(function(){
				...
				// console.log('urlStr :'+urlStr, _g.fileDir);
				if(urlStr.search(_g.fileDir) != -1){
		...
		, event_loadLayerPopHandler = function(){
			...
			rMap.tab_content.find('tbody tr.layer .path a').on('click' , function(){
				...
				if(_g.url_info.search('file') != -1) {//로컬로 볼경우
					goURL = _g.layerPopUrl.local + goURL;
				} else {//서버에서 볼경우s
					goURL = _g.layerPopUrl.guide + '?' + _g.layerPopUrl.server + goURL;
				}
				window.open(goURL);
				return false;
			});
	...
	var Export = (function(){
		var excelExport = function(){
			...
			$btn.on('click', function(){
				...
				// Popup.open('Export', copyTable() , function(){
	 ...
	 */
	var Log = (function(){
		var todoUrl ='./worklist.todo.html';

		var set = function($content){
			var $logCell = $content.find('td.log');
			$logCell.each(function(){
				...
				$desc.each(function(){
					...
					$i.each(function(){
						var _txt = $(this).text();
						$(this).wrap('<a href='+todoUrl+'?find='+_txt+' target="_blank">');
					})
				});
		...
		, setData = function($desc){
			...
			// html +='    <thead><tr><th scope="col">Date</th><th scope="col">Descrption</th></tr></thead>';
			html +='    <tbody>'

			$desc.find('> *').each(function(i){
				var day = $(this).find('time').text();
				var copy = $(this).clone();
				copy.find('time').empty().remove();
				var content = ltrim(copy.html());

				if(!day && !content){

				}else{
					if(!day) day = "N/A";
					html +='	  	<tr><th scipe="row">'+ day +'</th> <td>'+ content +'</td></tr>';
				}
			});
		...
		, openLogPop = function($target){
			$target.on('click', function(){
				_g.focusObj = $(this).closest('tr');
				var title = "Log";
				var data = $(this).parent().data('log');

				Popup.open('modal_log', title, data, function(){});
			});
		}

		, viewOneLine =  function($content){
			var $logCell = $content ? $content.find('td.log') : rMap.tab_content.find('td.log');
			$logCell.each(function(){
				// var $desc = $(this).find('.desc');
				var $desc = $(this).find('> *:not(.btn_more)');
				$desc.hide();
				if(!_g.is_mobile) $desc.eq(0).show();// 모바일 경우 처음부터 모두 hidden
			});
		}
		...
		, foldingHandler = function(){
			...
			if(_g.logObj.isShowAll) $logBtn.addClass('on');
			else $logBtn.removeClass('on')

			// var $desc = $tdLog.find('.desc');
			var $desc = $tdLog.find('> *:not(.btn_more)');
			$logBtn.off().on('click', function(){
				$logBtn.toggleClass('on')
				if($(this).hasClass('on')){
					_g.logObj.isShowAll = true;
					$desc.show();
				}else{
					_g.logObj.isShowAll = false;
					viewOneLine();
				}
	...
	var More = (function(){
		...
		*/
		...
		, initModule = function(){
			...
			// set({target: rMap.body.find('[data-ui="noti"]')}); //More
	 ...
	 */
	var Todo = (function(){
		//LinkInfo 영역의 Log 버튼 클릭시 Log 항목 show hide Toggle
		var logBtnHandler = function(_url){
			var $table = $('.tab_contents_wrap')
				, $logCell = $table.find('td.log')
				// , $desc = $logCell.find('.desc')
				, $desc = $logCell.find('> *:not(.btn_more)')
				, toggleFlag = true
			;

			$('.info_section .log_btn').on('click', function(){
				if(!toggleFlag){
					$logCell.each(function(){
						// var $this = $(this).find('.desc');
						var $this = $(this).find('> *:not(.btn_more)');
						if($this.size() >1){
							$this.hide();
							$this.eq(0).show();
						}
		...
		//현재 페이지가 todolist일 경우 todoID Search
		var todoSearch = function(){
			if(decodeURI(Util.getFileName()) != 'worklist.todo.html')  return false;
			...
			var findTxt = queryObj.find.trim();
			if(!!findTxt){
				setTimeout(function(){
					$trs.each(function(){
	...
	var Etc = (function(){
		var idSplit = function(){
			...
			$path.each(function(){
				var txt = $(this).text().replaceAll(' ', '').split('_')// underbar 뒷부부네 제외 위해
					, str = ''
				;

				if(txt[0].length > 0){
					if(start > 0 ) str +='<span>' + txt[0].substring(0, start) + '</span>';

					for(var i= start; i< txt[0].length; i++){
						if(i % num == 0) str +='<span>' + txt[0].substring(i, i+2) + '</span>';
					}

					$(this).empty().html('<span class="id_split">'+str + (!!txt[1] ? '_'+txt[1] : '' ) +'</span>');
				}
			});
		}
	...
	var Helper = (function(){
		...
		var sortDate = function(_AR){
			return _AR.sort(function(a, b) {
					var sortA = a;
					var sortB = b;
					var ARa = sortA.split(_g.dateObj.seperate);
					var ARb = sortB.split(_g.dateObj.seperate);
		...
		//프로젝트 시작일 종요일과 비교해서 기간내 산출
		var periodDate = function(a){
			var sortA = dateToNum(a);
			var sortB = dateToNum(_manmonth.projectStartDate);
			var sortC = dateToNum(_manmonth.projectEndDate);
	...
	var Report = (function(){
		...
		var setMap = function(_arg){
			reportMap ={
				reportTable_calendar : ''
					...
					+'    <div class="btn_area">'
					+'      <strong class="date_form  mt10"><span class="start_date">'+_manmonth.projectStartDate+'</span> ~ <span class="end_date">'+_manmonth.projectEndDate+'</span></strong>'
		...
		, setProjectDate = function(){
			_manmonth.projectStartDate = Data.dateFormat(_manmonth.projectStartDate);
			_manmonth.projectEndDate = Data.dateFormat(_manmonth.projectEndDate);

			if(!_g.dateObj.datePeriodFlag){
				_manmonth.projectStartDate_ORI = _manmonth.projectStartDate
				_manmonth.projectEndDate_ORI = _manmonth.projectEndDate
			}

			var ARstart = _manmonth.projectStartDate.split(_g.dateObj.seperate);
			var ARend = _manmonth.projectEndDate.split(_g.dateObj.seperate);

			//Error
			projectDateObj.error =false;
			if(Number(ARstart[0]) > Number(ARend[0])) projectDateObj.error = 'year';
			if(Number(ARstart[0]) == Number(ARend[0]) && Number(ARstart[1]) > Number(ARend[1])) projectDateObj.error = 'month';
			if(Number(ARstart[0]) == Number(ARend[0]) && Number(ARstart[1]) == Number(ARend[1]) && Number(ARstart[2]) > Number(ARend[2])) projectDateObj.error = 'date';

			projectDateObj.startYear = ARstart[0];
			projectDateObj.startMonth = ARstart[1];
			projectDateObj.startDate = ARstart[2];
			projectDateObj.endYear = ARend[0];
			projectDateObj.endMonth = ARend[1];
			projectDateObj.endDate = ARend[2];

			projectDateObj.totalYear = Number(projectDateObj.startYear);
			projectDateObj.totalMonth = Number(projectDateObj.startYear);
			projectDateObj.totalWeek = 0;
			projectDateObj.totalDate = 0;
			projectDateObj.totaHoliday  = 0;

			projectDateObj.ARyear = [Number(projectDateObj.startYear)];
			projectDateObj.ARmonth = [Number(projectDateObj.startMonth)];
			projectDateObj.ARweek = [];

			var periodYear = Number(projectDateObj.endYear - projectDateObj.startYear)
				, firstMonth = Number(projectDateObj.startMonth)
				, lastMonth = Number(projectDateObj.endMonth)
			...
			for(var yy =0; yy<= periodYear; yy++){
				...
				if(periodYear == 0){
					for(var i = (firstMonth + 1); i <= lastMonth; i++) {
						projectDateObj.ARmonth.push(i);
						projectDateObj.ARyear.push(Number(projectDateObj.startYear));
						monthCnt++;
					}
				}
				//project 기간이 1년 이상일때
					...
					if(yy ==0){// 첫해
						...
						for(var i = 1; i <= lastMonth; i++) {
							projectDateObj.ARmonth.push(i);
							projectDateObj.ARyear.push(Number(projectDateObj.startYear) + yearCnt)
							monthCnt++;
						}
					}else{//중간해(1년 초과)
						for(var i = 1; i <= 12; i++) {
			...
			projectDateObj.totalMonth = monthCnt;
			projectDateObj.totalYear = (monthCnt/12).toFixed(2);
			// if(String(realPercent).split('.')[1]) realPercent = realPercent.toFixed(2);

			// console.log(projectDateObj);
		}

		, setData = function(){
			var $trs = rMap.contentTrs;

			// [#comment: date_modify]보통 프로젝트가 1년단위 일경우라 생각하고 작업리스트에 날짜 삽입을 연을 삭제하고 기록 하였기 때문에..
			// 추후 1년 넘는 프로젝트일 경우 수정필요(22/01/03)
			var yearChangeNum = _g.dateObj.yearChangeNum; // 연단위 Change : 6~12월 : 2021, 1~5월 : 2022
			...
			var ARstartDate = _manmonth.projectStartDate.split(_g.dateObj.seperate);
			var ARendDate = _manmonth.projectEndDate.split(_g.dateObj.seperate);
			...
			for(var i=0; i<=ARdate.length - 1; i++){
				var ARnum = ARdate[i].split(_g.dateObj.seperate);
				var dayNum = ARnum[0]+ String(ARnum[1]).zf(2) + String(ARnum[2]).zf(2);//20210304
				var monthStr = ARnum[0]+_g.dateObj.seperate+ String(ARnum[1]).zf(_g.dateObj.zeroFill);
			...
			// console.log('3.2 ARdate:', ARdate);
			// console.log('4.ARmonth:', ARmonth);
			...
			// console.log('corder:', reportObj.corder, ARmonth);
			...
			for(var i = 0; i<=reportObj.corder.length-1; i++){
				$trs.each(function(ii){
					...
					if(corderTxt == reportObj.corder[i]){
						...
						//###담당자별 월취합
						if(!!strDate){
							var ARstr = strDate.split(_g.dateObj.seperate);
							str_month = ARstr[0]+_g.dateObj.seperate+ARstr[1];
				...
				for(var iii=0; iii <= reportObj.obj[i].day.length-1; iii++){
					var _AR = reportObj.obj[i].day[iii].label.split(_g.dateObj.seperate);
					var _target = _AR[0]+_g.dateObj.seperate+_AR[1];
			...
			//### 월별 검색할 날짜 Data
			projectDateObj.ARmonth_date=[] 
			for(var i = 0; i<= reportObj.month.length-1; i++){
				var mothStr= reportObj.month[i]
				projectDateObj.ARmonth_date[i] = new Array();

				for(var ii =0; ii<= reportObj.day.length-1; ii++){
					var ARdayStr = reportObj.day[ii].split(_g.dateObj.seperate)
					var dayStr = ARdayStr[0] + _g.dateObj.seperate + ARdayStr[1];
					if(mothStr == dayStr){
						projectDateObj.ARmonth_date[i].push(reportObj.day[ii])
					}
				}
			}
		...
		, table_calendar = function(){
			...
			var rowStart = 0; //가로
			var colStart = 0;
			var rowNum = reportObj.day.length-1; //가로
			var colNum = reportObj.corder.length-1; //세로

			var ARmonth = projectDateObj.ARmonth;
			var ARweekStr = ['일', '월', '화', '수', '목', '금', '토'];

			projectDateObj.ARweek_date =[]; //주당 날짜 삽입;
			...
			for(var i = 0; i < ARmonth.length;  i++){
				...
				var cruDate = new Date(projectDateObj.ARyear[i],monObj, 0);
				...
				//첫달
				if(i ==0) {
					startDate = Number(projectDateObj.startDate - 1);
					colgroupNum = lastDate - startDate;

					//week(첫째 주)
					var tmpDate = new Date(projectDateObj.ARyear[i], mon -1, projectDateObj.startDate);
					var strYear = projectDateObj.ARyear[i]+_g.dateObj.seperate+mon.zf(_g.dateObj.zeroFill)+_g.dateObj.seperate+projectDateObj.startDate.zf(_g.dateObj.zeroFill);
					startWeek = tmpDate.getDay();

					if(startWeek > 0){
						$tfoot.find('tr.tr_week').append('<th scope="col" data-date="'+strYear+'" data-week="1" colspan='+(7 - startWeek)+'>'+ 1 +'w <span class="sum link_num"></span></th>');
						projectDateObj.ARweek_date[1] = new Array();
					}
				}

				//마지막달
				if(i == ARmonth.length-1) {
					lastDate = (new Date(year, mon, Number(projectDateObj.endDate))).getDate();
				}

				// 구분/년 삽입
				var trYearStr = projectDateObj.ARyear[i] +_g.dateObj.seperate + mon.zf(_g.dateObj.zeroFill);
				...
				for(var ii = startDate; ii< lastDate;ii++) {
					var yearStr = projectDateObj.ARyear[i];
					var dateStr = String(ii+1).zf(_g.dateObj.zeroFill);
					var dateFullStr = yearStr +_g.dateObj.seperate+ monObj.zf(_g.dateObj.zeroFill) +_g.dateObj.seperate+ String(ii+1).zf(_g.dateObj.zeroFill); 
					var monDayStr = monObj.zf(_g.dateObj.zeroFill) +_g.dateObj.seperate+ String(ii+1).zf(_g.dateObj.zeroFill); 
					// 5월기준 연단위 변화 로 인하여 : 기본 작업리스트에 년도 삽입으로 변경 고려 
					...
					if(newDay == 6){// 토
						...
						holidayCnt++;
						//week: 주
						projectDateObj.ARweek_date[weekCnt+1]= new Array();
						weekCnt++;
						$tfoot.find('tr.tr_week').append('<th scope="col" data-date="'+(yearStr +_g.dateObj.seperate+ monObj +_g.dateObj.seperate+ (ii+2))+'"data-week="'+weekCnt+'" colspan="7">'+weekCnt+'w <span class="sum link_num"></span></th>');
					}else{
					...
					//주당 날짜 저장
					projectDateObj.ARweek_date[weekCnt].push(dateFullStr);

					dateCnt++;

					//공휴일 삽입 : Custom Holiday
					for(var hh =0; hh<= _manmonth.holiday.length-1; hh++){
						var holidayStr = Data.dateFormat(_manmonth.holiday[hh].date);
						var holidayLabel = _manmonth.holiday[hh].label;
			...
			projectDateObj.totalDate = dateCnt;
			projectDateObj.totalWeek = weekCnt;
			projectDateObj.totaHoliday = holidayCnt
			//=== End calendar
			...
			for(var i = colStart ; i<= colNum; i++){
				...
				//개인휴가 삽입
				for(var dd= 0; dd<=_manmonth.corder.length -1; dd++){
					if(_manmonth.corder[dd].name == corderStr) {
						for(var cc= 0; cc<=_manmonth.corder[dd].vacation.length -1; cc++){
							var vacationStr = Data.dateFormat(_manmonth.corder[dd].vacation[cc].date);
							var vacationLabel = _manmonth.corder[dd].vacation[cc].label;
							var vacationDay = _manmonth.corder[dd].vacation[cc].day;
			...
			//개인별 week CALC
			for(var i = 0 ; i<= projectDateObj.totalWeek; i++){
				//개인별 week
				for(var k = colStart ; k<= colNum; k++){
				...
				projectDateObj.ARweek[i] = weekSum_total;
			...
			function showItem_week($this){
				if($this.find('.sum').text().length>0){
					...
					var ARdateStr = projectDateObj.ARweek_date[idx_week];
					var corder = reportObj.corder[$this.parent().index()];
					var cnt = 1; 
		...
		, table_date = function(){
			...
			var rowStart = 0; //가로
			var rowEnd = reportObj.day.length-1; //가로
			var colStart = 0;
			var colNum = reportObj.corder.length-1; //세로
			...
			for(var i = rowStart ; i<= rowEnd; i++){
				...
				var ARdateYear = reportObj.day[i].split(_g.dateObj.seperate);
				var dateYear = ARdateYear[0]+_g.dateObj.seperate+ARdateYear[1];
			...
			for(var i = colStart ; i<= colNum; i++){
				...
				;
				$tbody.append(_tbody);
				$tbodyClone.append(_tbody);//clone
			}
			...
			function showItem($this){
				...
				if($this.text().length>0){
					...
					rMap.contentTrs.each(function(){
						$(this).hide();
						// var targetStr = $(this).find('.rdate').text();
		...
		, table_week = function(){
			var $reportArea = appendTable('week_report', "Week")
				...
				, rowStart = 0 //가로
				, colStart = 0
				, rowNum = reportObj.week.length-1 //가로
				, colNum = reportObj.corder.length-1 //세로
			...
			for(var i = rowStart ; i<= rowNum; i++){
				var weekNum = reportObj.week[i]
					, weekStart = projectDateObj.ARweek_date[weekNum][0]
					, weekEnd = projectDateObj.ARweek_date[weekNum][projectDateObj.ARweek_date[weekNum].length - 1]
				;
				$thead_ths += '<th scope="col" data-week="'+weekNum+'" data-week-start="'+weekStart+'" data-week-end="'+weekEnd+'"><a href="javascript:void(0);" class="btn_cal_move btn_week">'+reportObj.week[i]+'w</a></th>';
			...
			function showItem($this){
				...
				if($this.text().length>0){
					var idx = $this.index()
						...
						, ARdateStr = projectDateObj.ARweek_date[idx_week]
		...
		, table_month = function(){
			...
			var rowStart = 0; //가로
			var rowNum = reportObj.month.length-1; //가로
			var colStart = 0;
			var colNum = reportObj.corder.length-1; //세로
			...
			function showItem($this){
				...
				if($this.text().length>0){
					...
					rMap.contentTrs.each(function(){
						...
						if(!!monData){
							var ARdata = monData.split(_g.dateObj.seperate);
							var targetData = ARdata[0]+_g.dateObj.seperate+ARdata[1];

							projectDateObj.ARmonth_date[idx -1]

							if(totalFlag) {
								if(targetData == dataMonth) {
									if(dataMonth == firstMonth || dataMonth == lastMonth){
										for(var iii = 0; iii <= projectDateObj.ARmonth_date[idx -1].length-1; iii++){
											if(monData == projectDateObj.ARmonth_date[idx -1][iii]) showPage($(this), cnt++);
										}
		...
		, table_total = function(){
			...
			var $tbody =$table.find('.table_report_body');
			var $tfoot =$table.find('.table_report_foot');
			var calStr = '* '+Number(projectDateObj.totalYear)+'년('
				+ projectDateObj.totalMonth +'개월, '
				+ projectDateObj.totalWeek +'주,  '
				+ projectDateObj.totalDate +'일('
				+ '작업일+휴일='
				+ Number(projectDateObj.totalDate - projectDateObj.totaHoliday) +'+'
				+ projectDateObj.totaHoliday +')'
			;
			...
			for(var i = colStart ; i<= colNum; i++){
				...
				for(var dd= 0; dd<=_manmonth.corder.length -1; dd++){
					if(_manmonth.corder[dd].name == corderStr) {
						//투입, 철수일
						startDate = _manmonth.corder[dd].startDate = Data.dateFormat(_manmonth.corder[dd].startDate);
						endDate = _manmonth.corder[dd].endDate = Data.dateFormat(_manmonth.corder[dd].endDate);

						// console.log('input, output:', startDate, endDate)
						...
						var projectStartNum = Helper.dateToNum(_manmonth.projectStartDate);
						var projectEndNum = Helper.dateToNum(_manmonth.projectEndDate);
						if(inputNum <= projectStartNum) inputDate = _manmonth.projectStartDate;
						if(outputNum >= projectEndNum) outputDate = _manmonth.projectEndDate;
						...
						//작업자별 휴가
						for(var cc= 0; cc<= _manmonth.corder[dd].vacation.length -1; cc++){
							vacationNum += _manmonth.corder[dd].vacation[cc].day;//개인의 전체휴가
							var periodFlag = Helper.periodDate(_manmonth.corder[dd].vacation[cc].date);//개인의 프로젝트 기간내 휴가 true
							if(periodFlag) vacationNum_period += _manmonth.corder[dd].vacation[cc].day;
						}
					}
				}
			...
			// console.log('tmpNanNum :'+tmpNanNum)
			...
			var $tfoot_ths =''
				...
				+'<th scope="row" class="total period startdate" style="width:100px"><span class="link_num no_link">'+_manmonth.projectStartDate +'</span></th>' // 투입일
				+'<th scope="row" class="total period enddate" style="width:100px"><span class="link_num no_link">'+_manmonth.projectEndDate +'</span></th>' // 철수일
		...
		, table_handler = function(){
			// console.log('goCalendar:handler')
			//date
			$('.btn_cal_move.btn_date').on('click', function(){
				...
				var ARyearStr = $target.data('date').split(_g.dateObj.seperate);
				var yearStr = String(ARyearStr[0] +_g.dateObj.seperate + ARyearStr[1]);
			...
			function btnPeriodSet(){
				//periodBtn Set
				if(_g.dateObj.datePeriodFlag){//기간설정
					$periodBtn.removeClass('active')
					$totalBtn.show();
					$totalBtn.addClass('active')
					// $periodBtn.find('span').text('Total')
				}else{
					$totalBtn.hide();
					// $periodBtn.find('span').text('Period')
				}
			}
			...
			$calDate.find('.item').on('click', function(){
				...
				if($(this).hasClass('start')){
					...
					setStartDate = _manmonth.projectStartDate;
					setEndDate = _manmonth.projectEndDate;
			...
			$periodBtn.on('click', function(){
				if(calCnt >=2){
					_manmonth.projectStartDate = setStartDate
					_manmonth.projectEndDate = setEndDate
					_g.dateObj.datePeriodFlag = true;
			...
			$totalBtn.on('click', function(){
				if(_g.dateObj.datePeriodFlag){
					console.log(_manmonth.projectStartDate_ORI, _manmonth.projectEndDate_ORI);

					_manmonth.projectStartDate = _manmonth.projectStartDate_ORI
					_manmonth.projectEndDate = _manmonth.projectEndDate_ORI

					_g.dateObj.datePeriodFlag = false;
	...
	var initModule = function(options, callback){
		...
		Setting.initModule(function(){
			...
			if(Setting.bool(_g.settingObj.data.data)) Data.initModule();
			if(Setting.bool(_g.settingObj.data.table_folding)) TableFolding.initModule();// Data와 연결 되어 있음
			Nav.initModule();//2Depth show는 Data와 연결 되어 있음
			if(Setting.bool(_g.settingObj.data.filter)) Filter.initModule(); // ColorBox는 Data와 연결
			if(Setting.bool(_g.settingObj.option.component)) Component.initModule();
			...
			// Search_header.initModule();
			Log.initModule();//[22/12/06]: folding Btn insert 위해 Serch 뒤에 호출

			Export.initModule();

			if(Setting.bool(_g.settingObj.option.id_split)) Etc.idSplit();
			// SideNav.initModule(); //20211222 추후 작업예정

			if(Setting.bool(_g.settingObj.option.quick)) Quick.initModule();
		});
...
// 				data:['O','X','',''] 
// 				, option:['', true] 
// 				, table:[false,false,true,true,true,true, true,true, true, true, true] 
// 			}
// 			, headerFolding :{
// 				headerClose:true
// 			}
